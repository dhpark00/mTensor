...
다운로드한 `mTensor.zip`을 `<Wolfram Engine 설치 폴더>/AddOns/Applications`에 푼다. (참고) 윈도우에서 MathLink 라이브러리 호출에 문제가 생기면 파일 `ml64i4.dll`을 `...\mTensor\xPermML\LibraryResources` 폴더에 위치시킨다.
