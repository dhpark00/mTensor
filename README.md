...
다운로드한 `mTensor.zip`을 `<Wolfram Engine 설치 폴더>/AddOns/Applications`에 푼다. (참고) 윈도우에서 MathLink 라이브러리 호출에 문제가 생기면 *Mathematica*가 제공하는 파일 `ml64i4.dll`을 파일 `xPermML64.mswin`가 있는 폴더인 `...\mTensor\xPermML\LibraryResources`에 위치시킨다.
